...
pub(super) async fn run(
    ...
    let jobs = endpoints
        ...
        .filter_map(|endpoint| {
            let service = access_service(endpoint);
...
async fn check_one(
    ...
    if has_http {
        results.push(result(
            ...
            port,
            service,
            "Bounded HTTP metadata read",
            ServiceAccessStatus::Offered,
            "HTTP metadata reads are reported separately from handshake-only service checks",
            Vec::new(),
        ));
    }
    ...
    if matches!(
        ...
        results.push(result(
            ...
            port,
            service,
            "Pre-authentication observation",
            if service == ServiceKind::Rsync {
                ServiceAccessStatus::Offered
            } else {
                ServiceAccessStatus::Inconclusive
            },
            "The exposed service was recorded without enumeration or a data command",
            Vec::new(),
        ));
        return results;
    }
    ...
    if matches!(port, 465 | 636 | 993 | 995 | 3269 | 8883) {
        results.push(result(
            ...
            port,
            service,
            "TLS-wrapped pre-authentication observation",
            ServiceAccessStatus::Inconclusive,
            "TLS exposure was recorded, but no plaintext protocol message was sent to the encrypted listener",
            Vec::new(),
        ));
        return results;
    }
    if limiter.wait(cancel).await.is_err() {
        results.push(result(
            ...
            ServiceAccessStatus::Inconclusive,
            "Check cancelled before connection",
            Vec::new(),
        ));
        return results;
    }
    ...
    let Some(mut stream) = candidate.stream.take() else {
        results.push(result(
            ...
            "Handshake-only check",
            ServiceAccessStatus::Inconclusive,
            "Unable to establish the bounded negotiation session",
            candidate.attempt.error.into_iter().collect(),
        ));
        return results;
    };
    ...
    results.push(checked.unwrap_or_else(|error| {
        result(
            ...
            "Handshake-only check",
            ServiceAccessStatus::Inconclusive,
            "The bounded negotiation did not produce a conclusive result",
            vec![error],
        )
    }));
    results
...
async fn negotiate(
    ...
    match service {
        ServiceKind::Ftp => ftp(stream, ip, port).await,
        ServiceKind::Ldap => ldap(stream, ip, port).await,
        ServiceKind::Smb => smb(stream, ip, port).await,
        ServiceKind::Redis => redis(stream, ip, port).await,
        ServiceKind::Memcached => memcached(stream, ip, port).await,
        ServiceKind::Mqtt => mqtt(stream, ip, port).await,
        ServiceKind::Vnc => vnc(stream, ip, port).await,
        ServiceKind::Rdp => rdp(stream, ip, port).await,
        ServiceKind::Smtp => smtp(stream, ip, port).await,
        ServiceKind::Imap => imap(stream, ip, port).await,
        ServiceKind::Pop3 => pop3(stream, ip, port).await,
        ServiceKind::PostgreSql => postgres(stream, ip, port).await,
        ServiceKind::Mysql => mysql(stream, ip, port).await,
        ServiceKind::MongoDb => Ok(result(
            ip,
            port,
            service,
            "Pre-authentication observation",
            ServiceAccessStatus::Inconclusive,
            "No MongoDB data or enumeration command was sent",
            Vec::new(),
        )),
        ServiceKind::Rsync => Ok(result(
            ip,
            port,
            service,
            "Daemon greeting",
            ServiceAccessStatus::Offered,
            "rsync service negotiation was observed without enumerating modules",
            Vec::new(),
        )),
        ServiceKind::Rtsp => rtsp(stream, ip, port, hostname).await,
        ServiceKind::Rtmp => rtmp(stream, ip, port).await,
        _ => Ok(result(
            ip,
            port,
            service,
            "Pre-authentication observation",
            ServiceAccessStatus::Inconclusive,
            "The protocol cannot prove access without enumeration or a data operation",
            Vec::new(),
        )),
    }
}

async fn ftp(stream: &mut TcpStream, ip: IpAddr, port: u16) -> Result<ServiceAccessResult, String> {
    let _ = read_some(stream).await?;
    stream
        .write_all(b"USER anonymous\r\n")
        .await
        .map_err(io_error)?;
    let user = text(read_some(stream).await?);
    let status = ftp_status(&user);
    let accepted = if status == Some(230) {
        ...
        stream
            ...
            .await
            .map_err(io_error)?;
        ftp_status(&text(read_some(stream).await?)) == Some(230)
    } else {
        false
    };
    ...
    Ok(result(
        ...
        status
            .map(|value| vec![format!("FTP status {value}")])
            .unwrap_or_default(),
    ))
}
...
async fn ldap(
    ...
    stream
        ...
        .await
        .map_err(io_error)?;
    let response = read_some(stream).await?;
    let code = response
        .windows(3)
...
async fn redis(
    ...
    let response = text(read_some(stream).await?);
...
async fn memcached(
    ...
    let confirmed = text(read_some(stream).await?)
        .to_ascii_lowercase()
        .starts_with("version ");
    let _ = stream.write_all(b"quit\r\n").await;
    Ok(result(
        ...
        "Unauthenticated version command",
        if confirmed {
            ServiceAccessStatus::Confirmed
        } else {
            ServiceAccessStatus::Inconclusive
...
async fn vnc(stream: &mut TcpStream, ip: IpAddr, port: u16) -> Result<ServiceAccessResult, String> {
    let banner = read_some(stream).await?;
    if !banner.starts_with(b"RFB ") {
        return Err("VNC version banner was not recognized".to_owned());
    }
    let version = &banner[..banner.len().min(12)];
    stream.write_all(version).await.map_err(io_error)?;
    let security = read_some(stream).await?;
...
async fn rdp(stream: &mut TcpStream, ip: IpAddr, port: u16) -> Result<ServiceAccessResult, String> {
...
async fn smtp(
    ...
    stream
        ...
        .await
        .map_err(io_error)?;
    let capabilities = text(read_some(stream).await?).to_ascii_uppercase();
    let auth = auth_mechanisms(&capabilities);
    let _ = stream.write_all(b"QUIT\r\n").await;
...
async fn postgres(
    ...
    Ok(result(
        ...
        "SSLRequest",
        if matches!(response.first(), Some(b'S' | b'N')) {
            ServiceAccessStatus::Offered
        } else {
            ServiceAccessStatus::Inconclusive
        },
        "PostgreSQL pre-authentication transport method recorded; no startup credentials or query were sent",
        Vec::new(),
    ))
}
...
async fn mysql(
    ...
    let greeting = read_some(stream).await?;
...
async fn rtsp(
    ...
    stream
        ...
        .map_err(io_error)?;
    let response = read_some(stream).await?;
    let status = rtsp_status(&response);
    Ok(result(
        ip,
        port,
        ServiceKind::Rtsp,
        ...
        "RTSP signalling was checked without DESCRIBE, SETUP, PLAY, or media access",
        status
...
async fn rtmp(
    ...
    Ok(result(
        ...
        "RTMP protocol negotiation was attempted without a stream key or media request",
        if recognized {
...
fn rtsp_status(response: &[u8]) -> Option<u16> {
    let line_end = response.windows(2).position(|window| window == b"\r\n")?;
    let first_line = std::str::from_utf8(&response[..line_end]).ok()?;
...
fn valid_rtmp_handshake(response: &[u8], request: &[u8]) -> bool {
    response.len() == 3073
        && request.len() == 1537
        && response[0] == 3
...
async fn smb(stream: &mut TcpStream, ip: IpAddr, port: u16) -> Result<ServiceAccessResult, String> {
    let negotiate = smb_negotiate_packet();
    stream.write_all(&negotiate).await.map_err(io_error)?;
    let response = read_some(stream).await?;
    if response.len() < 72 || response.get(4..8) != Some(&[0xfe, b'S', b'M', b'B']) {
        return Err("SMB2 negotiate response was not recognized".to_owned());
    }
    let session = smb_session_packet();
    stream.write_all(&session).await.map_err(io_error)?;
    let response = read_some(stream).await?;
    let status = response
        .get(12..16)
    ...
    Ok(result(
        ...
        port,
        ServiceKind::Smb,
        "SMB2 negotiate and anonymous session setup",
        if status == Some(0) {
            ServiceAccessStatus::Confirmed
...
fn smb_negotiate_packet() -> Vec<u8> {
    let mut packet = vec![0u8; 4 + 64 + 38];
...
fn smb_session_packet() -> Vec<u8> {
    let mut packet = vec![0u8; 4 + 64 + 24];
    let length = packet.len() as u32 - 4;
    packet[0..4].copy_from_slice(&length.to_be_bytes());
    packet[4..8].copy_from_slice(&[0xfe, b'S', b'M', b'B']);
    packet[8..10].copy_from_slice(&64u16.to_le_bytes());
...
async fn read_some(stream: &mut TcpStream) -> Result<Vec<u8>, String> {
    let mut value = vec![0u8; READ_LIMIT];
    let length = stream.read(&mut value).await.map_err(io_error)?;
...
fn ftp_status(value: &str) -> Option<u16> {
    value.get(..3)?.parse().ok()
}

fn auth_mechanisms(capabilities: &str) -> Vec<String> {
    capabilities
        .lines()
...
fn access_service(endpoint: &EndpointScan) -> ServiceKind {

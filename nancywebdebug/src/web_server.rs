    ...
    for (name, value) in headers {
        ...
        if matches!(
            ...
            for matched in catalog_matches(value) {
                let matched = if matched.entry.identifier == "falcon-ruby" && header != "server" {
                    Match {
                        entry: entry("falcon-python").unwrap_or(matched.entry),
                        ..matched
                    }
                } else {
                    matched
                ...
                record(
                    ...
                    include_evidence.then(|| {
                        response_evidence(status, response_url, &format!("{name}: {value}"))
                    }),
                );
            }
            if header == "server"
                && let Some(version) = php_development_server_version(value)
                && let Some(entry) = entry("php-development-server")
            {
                record(
                    &mut detections,
                    entry,
            ...
            && let Some(entry) = entry(identifier)
        {
            record(
                ...
                FingerprintConfidence::Medium,
                include_evidence
                    .then(|| response_evidence(status, response_url, &format!("{name}: {value}"))),
            );
        }
                ...
                .map(|(name, _)| name.trim())
        {
            ...
            if let Some(entry) = product.and_then(entry) {
                record(
                    &mut detections,
                    entry,
                    None,
...
fn php_development_server_version(value: &str) -> Option<String> {
    let lower = value.to_ascii_lowercase();
    let remainder = lower
        .strip_prefix("php/")
    ...
    (suffix.starts_with("development server") && exact_exposed_version(&version)).then_some(version)
}
...
fn detect_body(
    ...
    if status.is_some_and(|status| apache_fallback_error_document(status, &lower))
        && let Some(entry) = entry("apache-httpd")
    {
        record(
            detections,
            entry,
            ...
            include_evidence.then(|| {
                response_evidence(
                    status,
                    response_url,
                    "Response body matches Apache's fallback ErrorDocument page",
                )
            }),
        );
    }
...
fn apache_fallback_error_document(status: u16, body: &str) -> bool {
...
fn response_evidence(status: Option<u16>, response_url: Option<&str>, detail: &str) -> String {
    let url = response_url
        .filter(|url| !url.is_empty())
        .map(sanitize_response_url);
    match (status, url) {
...
fn sanitize_response_url(value: &str) -> String {
